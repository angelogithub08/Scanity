<template>
  <DefaultPage>
    <template #header>
      <ListPageHeader title="Usuários" @filtrate="filtrate" @add="add" :show-add-button="false" />
    </template>
    <template #content>
      <DefaultTable
        :rows="rows"
...
const columns = ref<QTableColumn[]>([
  ...
  { name: 'active', label: 'Ativo', field: (row) => (row.active ? 'Sim' : 'Não'), align: 'left' },
  {
    name: 'created_at',
    label: 'Criado em',
    field: 'created_at',
...
async function onRequest(requestProp: RequestProp) {
  ...
  try {
    ...
    localStorage.setItem('users:list:pagination', JSON.stringify(pagination.value));
...
async function viewRecord(record: any) {
  await router.push({ name: 'admin-user', params: { id: record.id } });
}
...
onMounted(async () => {
  const paginationData = localStorage.getItem('users:list:pagination');
  if (paginationData) {
