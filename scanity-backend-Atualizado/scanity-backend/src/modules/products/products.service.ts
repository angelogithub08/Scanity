...
export class ProductsService {
  ...
  async create(createDto: CreateProductsDto): Promise<Product> {
    try {
      let barcode = createDto.barcode;

      if (!barcode) {
        const { barcode: uniqueBarcode } = await this.generateUniqueBarcode(
  ...
  async generateUniqueBarcode(accountId: string): Promise<{ barcode: string }> {
    const { barcode } = this.generateBarcode();
    const exists = await this.productsRepository.findOneByBarcode(
      accountId,
      barcode,
    );

    this.logger.debug(
      `Gerando código de barras: ${barcode}, existe? ${!!exists}`,
    );

    if (exists) {
      return await this.generateUniqueBarcode(accountId);
    }
  ...
  async update(id: string, updateDto: UpdateProductsDto): Promise<Product> {
    try {
      if (updateDto.barcode) {
        const existingProduct = await this.productsRepository.findOneByBarcode(
  ...
  private generateBarcode(): { barcode: string } {
    const random = Math.floor(Math.random() * 10 ** 15)
      .toString()
      .padStart(15, '0');
    return { barcode: random };
  }
