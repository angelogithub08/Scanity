...
export class AccountsRepository {
  ...
  /**
   * Busca conta por hash de email (comparação exata).
   *
   * O parâmetro `hashedEmail` já deve vir hasheado (sha256 com secret),
   * pois é o valor armazenado na coluna `email` da tabela `accounts`.
   *
   * INCLUI registros soft-deleted (deleted_at NOT NULL), porque a UNIQUE
   * constraint do banco age sobre todas as linhas — inclusive as deletadas.
   * Usar este método na checagem prévia evita violações de UNIQUE no INSERT.
   */
  async findByEmailHash(hashedEmail: string): Promise<Account | null> {
    this.logger.debug(`Buscando account por hash de email: ${hashedEmail}`);
    try {
      ...
      this.logger.error(
        `Erro ao buscar account por email hash: ${(error as Error).message}`,
        error instanceof Error ? error.stack : undefined,
      );
      throw error;
    }
  ...
  async remove(id: string, trx?: Knex.Transaction): Promise<number> {
