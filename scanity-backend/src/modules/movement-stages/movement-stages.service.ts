...
export class MovementStagesService {
  ...
  async create(createDto: CreateMovementStagesDto): Promise<MovementStage> {
    try {
      const { name, account_id } = createDto;
      const entries: CreateMovementStagesDto[] = [
        { name: `Entrada ${name}`, account_id },
        { name: `Saída ${name}`, account_id },
      ];

      const [first] = await this.movementStagesRepository.createMany(entries);
      this.logger.log(
        `MovementStages criados: Entrada ${name}, Saída ${name}`,
      );
      return first;
    } catch (error) {
      this.logger.error(
