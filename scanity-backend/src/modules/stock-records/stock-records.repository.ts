  ...
  async findAll(
    ...
    try {
      ...
      const data = (await this.defaultQuery(filters)
        ...
        .leftJoin('users', 'users.id', 'stock_records.user_id')
        .select([
          ...this.defaultSelectFields,
          'movement_stages.name as movement_stage_name',
  ...
  async removeByAccountId(
    ...
    try {
      ...
      const count = await connector(this.table)
        .whereIn('stock_id', function () {
          this.select('stocks.id')
            .from('stocks')
