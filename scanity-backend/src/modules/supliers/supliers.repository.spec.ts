...
describe('SupliersRepository', () => {
  ...
  describe('findAll', () => {
    it('should return paginated results', async () => {
      const qb = mockKnex();
      qb.first.mockResolvedValueOnce({ count: 3 });
      qb.offset.mockResolvedValueOnce([mockSuplier]);
    ...
    it('should apply filters correctly', async () => {
      ...
      expect(qb.where).toHaveBeenCalledWith('supliers.name', 'ilike', '%test%');
      expect(qb.where).toHaveBeenCalledWith('supliers.account_id', 'acc-1');
      expect(qb.whereNull).toHaveBeenCalledWith('supliers.deleted_at');
    });
  ...
  describe('list', () => {
    it('should return all records matching filters', async () => {
      const qb = mockKnex();
      qb.select.mockResolvedValueOnce([mockSuplier]);
  ...
  describe('findOne', () => {
    it('should find a suplier by id', async () => {
      const qb = mockKnex();
      qb.first.mockResolvedValueOnce(mockSuplier);

      const result = await repository.findOne('1');

      expect(qb.where).toHaveBeenCalledWith('supliers.id', '1');
      expect(qb.first).toHaveBeenCalled();
      expect(result).toEqual(mockSuplier);
    });

    it('should throw NotFoundException when suplier is not found', async () => {
  ...
  describe('create', () => {
    it('should insert data with returning and return the inserted record', async () => {
      const inserted = { ...mockSuplier, id: 'new-id' };
      mockKnex().returning.mockResolvedValueOnce([inserted]);
      const dto = { name: 'New Suplier', account_id: 'acc-1' };

      const result = await repository.create(dto as any);

      expect(mockKnex).toHaveBeenCalledWith('supliers');
  ...
  describe('update', () => {
    ...
    it('should update data and return the refreshed record', async () => {
      const updated = { ...mockSuplier, name: 'Updated' };
      const qb = mockKnex();
  ...
  describe('removeByAccountId', () => {
    it('should use trx when provided', async () => {
      ...
      expect(trx).toHaveBeenCalledWith('supliers');
