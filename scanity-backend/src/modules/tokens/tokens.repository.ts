...
export class TokensRepository {
  ...
  async revokeAndRemoveByAccountId(
    accountId: string,
    trx?: Knex.Transaction,
  ): Promise<{ revoked: number; deleted: number }> {
    this.logger.debug(`Revogando e removendo tokens da conta: ${accountId}`);
    try {
      const connector = trx ?? this.knex;
      const now = new Date();
      const revoked = await connector(this.table)
        .where({ account_id: accountId })
        .whereNull('revoked_at')
        .update({ revoked_at: now, updated_at: now });

      const deleted = await connector(this.table)
        .where({ account_id: accountId })
        .whereNull('deleted_at')
        .update({ deleted_at: now, updated_at: now });

      this.logger.debug(
        `Tokens da conta ${accountId}: ${revoked} revogados, ${deleted} removidos`,
      );
      return { revoked, deleted };
    } catch (error) {
      this.logger.error(
        `Erro ao revogar/remover tokens da conta ${accountId}: ${(error as Error).message}`,
        error instanceof Error ? error.stack : undefined,
      );
      throw error;
    }
