...
jest.mock('../../utils/encrypt.util', () => ({
  generateHash: jest
    ...
    .mockImplementation((str: string) =>
      Promise.resolve(`$2b$10$${str}_hashed`),
    ),
  generateEmailHash: jest.fn().mockImplementation((email: string) => `hashed-${email}`),
}));
...
describe('UsersService', () => {
      ...
      providers: [
        UsersService,
        {
          ...
          useValue: {
            ...
            deactivateByAccountId: jest.fn().mockResolvedValue(5),
          },
        },
      ],
    }).compile();
  ...
  describe('create', () => {
    ...
    it('should throw BadRequestException when password is not provided', async () => {
      const dtoWithoutPassword: CreateUsersDto = {
        name: 'No Password',
      ...
      await expect(service.create(dtoWithoutPassword)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.create(dtoWithoutPassword)).rejects.toThrow(
  ...
  describe('update', () => {
    ...
    it('should hash password and not pass it directly to repository', async () => {
      await service.update('mock-id', mockUpdateDto);

      const callArg = (repository.update as jest.Mock).mock.calls[0][1];
      expect(callArg).not.toHaveProperty('password', mockUpdateDto.password);
      expect(callArg).toHaveProperty('password', expect.any(String));
    });

    it('should hash email when updating without password', async () => {
      const emailOnlyDto = { email: 'new@email.com' };
      await service.update('mock-id', emailOnlyDto);

      const callArg = (repository.update as jest.Mock).mock.calls[0][1];
      expect(callArg).toHaveProperty('email', 'hashed-new@email.com');
    });
  });
  ...
  describe('deactivateByAccountId', () => {
    ...
    it('should return number of deactivated users', async () => {
      const result = await service.deactivateByAccountId(accountId);

      expect(repository.deactivateByAccountId).toHaveBeenCalledWith(
        accountId,
        undefined,
      );
    ...
    it('should pass trx parameter when provided', async () => {
      const trx = { transaction: true } as any;
      await service.deactivateByAccountId(accountId, trx);
    ...
    it('should throw BadRequestException when repository throws', async () => {
      jest
        .spyOn(repository, 'deactivateByAccountId')
        .mockRejectedValueOnce(new Error('Database error'));

      await expect(service.deactivateByAccountId(accountId)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
