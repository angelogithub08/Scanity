  ...
  <q-dialog v-model="dialogOpen" persistent>
    <q-card style="min-width: 400px">
      ...
      <q-card-section>
        ...
        <div class="q-mt-md">
          <div class="text-caption text-grey-7 q-mb-xs">Etapa (opcional)</div>
          <q-select
            v-model="movementStageId"
            :options="movementStagesOptions"
            option-label="name"
            option-value="id"
            emit-value
            map-options
            placeholder="Selecione a etapa de movimentação"
            bg-color="white"
            outlined
            dense
            clearable
...
async function submitQuickMovement() {
  ...
  try {
    await stocksResource.quickMovement({
      ...
      ...(movementStageId.value && { movement_stage_id: movementStageId.value }),
    });
