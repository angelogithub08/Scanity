  ...
  <DefaultPage>
    <template #outside-card>
      <q-breadcrumbs class="q-mb-md">
        ...
        <q-breadcrumbs-el label="Permissões de Perfil" />
      </q-breadcrumbs>
    </template>
    <template #header>
      <ListPageHeader
        ...
        @filtrate="filtrate"
        @add="add"
        add-permission="PROFILE_PERMISSIONS_CREATE"
      >
      </ListPageHeader>
    </template>
    <template #content>
      <DefaultTable
        ...
        delete-permission="PROFILE_PERMISSIONS_DELETE"
        row-clickable
...
const columns = ref<QTableColumn[]>([
  ...
  { name: 'profile_id', label: 'Profile Id', field: 'profile_id', align: 'left' },
  { name: 'permission_id', label: 'Permission Id', field: 'permission_id', align: 'left' },
...
async function add() {
  await router.push({ name: 'profile-permission' });
}

async function viewRecord(record: any) {
  await router.push({ name: 'profile-permission', params: { id: record.id } });
}
