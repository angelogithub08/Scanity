<template>
  <DefaultPage>
    <template #header>
      <div class="row items-center justify-between q-mb-md">
        <div>
          <div class="text-h6">Produtos abaixo do estoque mínimo</div>
          <div class="text-caption text-grey-7">
            Lista de produtos cuja quantidade atual está abaixo da quantidade mínima.
          </div>
        </div>
  ...
  <StockBelowMinimumReportFiltersDrawer
...
import StockBelowMinimumReportFiltersDrawer from 'src/components/reports/stock-below-minimum-report/StockBelowMinimumReportFiltersDrawer.vue';
...
const { print } = usePagedPrint({
  title: 'Relatório - Produtos abaixo do estoque mínimo',
});
const { downloadFromHttpResponse } = useDownload();

const printAreaRef = ref<HTMLElement | null>(null);
const filtersDrawerRef = ref<InstanceType<typeof StockBelowMinimumReportFiltersDrawer> | null>(
  null,
);
const loading = ref(false);
const exporting = ref(false);
const rows = ref<StockBelowMinimumReportItem[]>([]);
const categoryOptions = ref<{ id: string; name: string }[]>([]);
const productOptions = ref<{ id: string; name: string }[]>([]);

const filters = ref<StockBelowMinimumReportParams>({});
...
async function loadReport() {
  ...
  try {
    ...
    const params: StockBelowMinimumReportParams = { account_id };
    ...
    const { data } = await reportsResource.getStockBelowMinimum(params);
...
async function handleExportExcel() {
  ...
  try {
    ...
    const response = await reportsResource.getStockBelowMinimumExport(params);
    downloadFromHttpResponse(response, 'relatorio-produtos-abaixo-estoque-minimo.xlsx');
  } catch (error) {
    showError(error);
  } finally {
...
function getAppliedFilters(): StockBelowMinimumReportParams {
  const drawerFilters = filtersDrawerRef.value?.formattedFilters;
  const parsedFilters: StockBelowMinimumReportParams = {};
