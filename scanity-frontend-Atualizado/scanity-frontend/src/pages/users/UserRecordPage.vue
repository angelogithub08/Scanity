...
const profilesResource = useProfilesResource();
...
const profilesOptions = ref<Partial<Profile>[]>([]);
const currentAccount = computed(() => accountsStore.currentAccount);

async function loadProfiles() {
  const response = await profilesResource.list({ account_id: currentAccount.value?.id });
  profilesOptions.value = response.data;
}
