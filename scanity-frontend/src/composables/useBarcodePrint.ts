...
export function useBarcodePrint(options: UseBarcodePrintOptions = {}) {
  ...
  /**
   * Imprime múltiplas etiquetas QR organizadas em grid na folha A4.
   * Cada etiqueta tem 4cm x 4cm e exibe o QR code + texto do código de barras.
   * @param props - Objeto com title e barcode
   * @param count - Quantidade de etiquetas a imprimir
   */
  async function printMultipleBarcodesGrid(props: BarcodePrintProps, count: number): Promise<void> {
    const barcodeValue = (props.barcode ?? '').toString().trim();
    if (!barcodeValue) {
      console.warn('useBarcodePrint: produto sem código de barras.');
      return;
    ...
    for (let i = 0; i < count; i++) {
      labelsHtml += `
        <div class="label">
          <img src="${barcodeDataUrl}" alt="QR ${escapedBarcode}" />
          <div class="label-barcode">${escapedBarcode}</div>
        </div>`;
    }
