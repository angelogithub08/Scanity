  ...
  <DefaultPage>
    <template #header>
      <ListPageHeader
        ...
        @filtrate="filtrate"
        @add="add"
        add-permission="CUSTOMERS_CREATE"
      />
    </template>
    <template #content>
      <DefaultTable
...
const columns = ref<QTableColumn[]>([
  ...
  { name: 'name', label: 'Nome', field: 'name', align: 'left' },
  {
    name: 'document',
    label: 'Documento',
    field: (row) => applyMask(row.document, 'document'),
    align: 'left',
  },
  {
    name: 'phone',
    label: 'Telefone',
    field: (row) => applyMask(row.phone, 'phone'),
    align: 'left',
  },
...
async function viewRecord(record: any) {
  await router.push({ name: 'customer', params: { id: record.id } });
}
