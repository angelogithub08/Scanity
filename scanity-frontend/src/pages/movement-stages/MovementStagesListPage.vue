<template>
  <DefaultPage>
    <template #header>
      <ListPageHeader title="Etapas de Movimentação" @filtrate="filtrate" @add="add" />
    </template>
    <template #content>
      <DefaultTable
        :rows="rows"
...
async function add() {
  await router.push({ name: 'movement-stage' });
}

async function viewRecord(record: any) {
  await router.push({ name: 'movement-stage', params: { id: record.id } });
}
