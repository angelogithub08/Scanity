<template>
    ...
    <template #header>
      ...
      >
        <template #append_buttons>
          <q-btn
            v-if="isEditMode && productBarcode"
            icon="qr_code"
            color="primary"
            flat
            class="q-ml-sm"
          ...
          >
            <q-tooltip>Ver QR Code</q-tooltip>
          </q-btn>
          <q-btn
            v-if="isEditMode && productBarcode"
            icon="print"
          ...
          >
            <q-tooltip>Imprimir QR Codes</q-tooltip>
          </q-btn>
        </template>
      </RecordPageHeader>
    </template>
  ...
  <PrintQrCodesDialog
    v-model="showPrintQrDialog"
    :title="record.product_name ?? 'Produto'"
    :barcode="productBarcode"
    :default-quantity="printDefaultQuantity"
  />
</template>
...
import PrintQrCodesDialog from 'src/components/shared/PrintQrCodesDialog.vue';
...
const showPrintQrDialog = ref(false);
const stockRecordsTableRef = ref<InstanceType<typeof StockRecordsTable>>();

const productBarcode = computed(() => (record as any).product_barcode ?? null);

/** Quantidade sugerida para impressão de QR codes */
const printDefaultQuantity = ref(1);
...
const stockRecordsColumns = ref<QTableColumn[]>([
  ...
  {
    name: 'movement_stage_name',
    label: 'Etapa',
    field: 'movement_stage_name',
    align: 'left',
  },
  {
    name: 'user_name',
...
async function save() {
  try {
    ...
    if (valid) {
      ...
      // Se estava criando um novo estoque, oferece impressão de QR codes
      if (
        wasCreating &&
        ...
        printDefaultQuantity.value = record.current_quantity;
        await nextTick();
        showPrintQrDialog.value = true;
      }
    }
  } catch (error) {
    showError(error);
...
async function handleStockRecordCreated(data: Partial<StockRecord>) {
  try {
    ...
    const response = await stockRecordsResource.create(recordData);
    const createdRecord = response.data as Partial<StockRecord>;
    const isEntry = createdRecord?.type === 'ENTRADA';
    const entryQuantity = isEntry && createdRecord?.quantity ? Math.abs(createdRecord.quantity) : 0;
    ...
    await loadStockRecords();
    const stockResponse = await resource.findById(record.id as string);
    Object.assign(record, stockResponse.data);

    // Se foi uma entrada, oferece impressão de QR codes
    if (isEntry && entryQuantity > 0 && productBarcode.value) {
      Loading.hide();
      printDefaultQuantity.value = entryQuantity;
      await nextTick();
      showPrintQrDialog.value = true;
    }
  } catch (error) {
