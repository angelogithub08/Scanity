  ...
  <DefaultPage>
    ...
    <template #content>
      ...
      >
        ...
        <template #body-cell-qr_actions="props">
          <q-td :props="props" style="padding: 0px; margin: 0px" width="1%">
            <q-btn
              v-if="props.row.product_barcode"
              flat
              dense
              round
              icon="qr_code"
              color="primary"
              @click="showQrCodeForRow(props.row)"
            >
            ...
            <q-btn
              ...
              icon="print"
              color="primary"
              @click="showPrintQrForRow(props.row)"
            >
      ...
      <QrCodeDialog
        ...
        :title="selectedRow?.product_name ?? 'Produto'"
        :barcode="selectedRow?.product_barcode ?? null"
      />
      ...
      <PrintQrCodesDialog
        ...
        :title="selectedRow?.product_name ?? 'Produto'"
        :barcode="selectedRow?.product_barcode ?? null"
        :default-quantity="selectedRow?.current_quantity ?? 1"
      />
    </template>
  </DefaultPage>
</template>
...
function showQrCodeForRow(row: any) {
  selectedRow.value = row;
  showQrCodeDialog.value = true;
}

function showPrintQrForRow(row: any) {
  selectedRow.value = row;
